...
window.onscroll = function() {
  if (window.pageYOffset > sticky) {
    content.header.classList.add("sticky");
		content.backTop.style.display = "block";
  } else {
    content.header.classList.remove("sticky");
		content.backTop.style.display = "none";
  }
};

content.backTop.addEventListener("click",function(){
	window.scrollTo({
		top: 0,
		behavior: "smooth"
	});
  ...
  cards[i].firstElementChild.addEventListener("click", function() {
    ...
    // toggle click again
      ...
      while (nextSibling) {
        ...
        nextSibling.style.boxShadow = "0px -4px 16px 5px rgba(0, 0, 0, 0.2)";
        nextSibling.style.transition = "top .4s ease-in-out";
        nextSibling = nextSibling.nextElementSibling;
      }
    }
